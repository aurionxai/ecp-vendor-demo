# Margin Autopilot — copilot prototype

A runnable prototype of the **Claude-API copilot** layer for the Margin Autopilot
(see the mockup's Admin tab and `ECP-SCOPE.md` §11).

**The design point:** the LLM is the *interface*, not the calculator. Claude
(`claude-opus-4-8`) parses a natural-language question and calls a **deterministic
tool** (`propose_scrub` / `get_vendor_margin`) that runs the real billing math.
Every margin and scrub number comes from the tool — the model never invents one.
It only **recommends**; a human approves in the UI, and changes apply to the
**open week** only (never a locked invoice).
...
node margin-copilot.mjs "which vendors are losing us money and how do I fix them?"
node margin-copilot.mjs            # default: triage all vendors vs a 30% target
```
